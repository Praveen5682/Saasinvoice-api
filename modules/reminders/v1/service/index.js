...
module.exports.triggerReminder = async (id, userId) => {
  try {
    ...
    if (emailSent) {
      const existing = await db.reminders.findFirst({ where: { invoice_id: invoiceId } });
      if (existing) {
        await db.reminders.update({ where: { id: existing.id }, data: { status: "sent", last_sent: new Date() } });
      } else {
        await db.reminders.create({
          data: {
            invoice_id: invoiceId, user_id: userId, type: "email",
            status: "sent", last_sent: new Date(), reminder_date: new Date(),
          },
        });
      }
...
module.exports.processReminders = async () => {
  try {
    ...
    await db.reminders.updateMany({
      where: { status: "pending", reminder_date: { lt: new Date(today) } },
      data: { status: "overdue" },
    });

    const remindersToSend = await db.$queryRaw`
    ...
    for (const reminder of remindersToSend) {
      if (reminder.type === "email" && reminder.client_email) {
        ...
        await db.reminders.update({
          ...
          data: { status: newStatus, last_sent: emailSent ? new Date() : undefined },
        });
